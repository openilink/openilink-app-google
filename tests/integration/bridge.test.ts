...
/**
 * Google Workspace App 桥接集成测试
 * 需要手动启动 Mock Server 和 App 服务后才能运行
 *
 * 启动步骤：
 * 1. 启动 Mock Server:
 *    go run github.com/openilink/openilink-hub/cmd/appmock@latest \
 *      --listen :9801 --webhook-url http://localhost:8086/hub/webhook --app-token mock_app_token
 * 2. 启动 App:
 *    GOOGLE_CLIENT_ID=mock_id GOOGLE_CLIENT_SECRET=mock_secret GOOGLE_REFRESH_TOKEN=mock_token npm run dev
 * 3. 运行测试:
