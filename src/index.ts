...
async function onCommand(event: HubEvent, installationId: string): Promise<string | ToolResult> {
  ...
  if (localCfg) {
    console.log(`[App] 使用安装 ${installationId} 的本地加密配置`);
  }
...
// 模式 2: Hub 直接安装通知（POST /oauth/callback）
  ...
  mode2Hub.fetchConfig()
    .then((remoteCfg) => {
      if (Object.keys(remoteCfg).length > 0) {
        ...
        console.log("[App] 模式2: 已拉取并加密保存配置:", data.installation_id);
      }
    })
    .catch((err) => console.error("[App] 模式2: 拉取配置失败:", err));
  // 异步同步 tools 到 Hub
