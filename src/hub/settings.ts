/**
 * /settings 页面 — 允许用户通过 app_token 验证身份后查看和修改 Google 凭证配置
 */
...
<head>
  ...
  <title>Google Workspace — 设置</title>
  <style>${COMMON_STYLE}</style>
</head>
<body>
  <div class="card">
    <h1>Google Workspace 设置</h1>
...
/**
 * POST /settings/save — 保存修改后的 Google 凭证配置
 */
export async function handleSettingsSave(
  ...
  if (googleClientId) newConfig.google_client_id = googleClientId;
  if (googleClientSecret) newConfig.google_client_secret = googleClientSecret;
  if (googleRefreshToken) newConfig.google_refresh_token = googleRefreshToken;
  // redirect_uri 允许清空，始终更新
  newConfig.google_redirect_uri = googleRedirectUri;
...
function renderConfigForm(
  ...
  const maskedClientId = maskValue(cfg.google_client_id || "");
  const maskedClientSecret = maskValue(cfg.google_client_secret || "");
  const maskedRefreshToken = maskValue(cfg.google_refresh_token || "");
  const maskedRedirectUri = maskValue(cfg.google_redirect_uri || "");
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Google Workspace — 修改配置</title>
  <style>${COMMON_STYLE}</style>
</head>
<body>
  <div class="card">
    <h1>Google Workspace 设置</h1>
    <p class="desc">修改您的 Google OAuth 凭证配置</p>
    ${successMsg ? `<div class="success">${successMsg}</div>` : ""}

    <div class="current-config">
      <dt>当前 Client ID</dt>
      <dd>${maskedClientId || "（未配置）"}</dd>
      <dt>当前 Client Secret</dt>
      <dd>${maskedClientSecret || "（未配置）"}</dd>
      <dt>当前 Refresh Token</dt>
      <dd>${maskedRefreshToken || "（未配置）"}</dd>
      <dt>当前 Redirect URI</dt>
      <dd>${maskedRedirectUri || "（未配置）"}</dd>
    </div>
    ...
    <form method="POST" action="/settings/save">
      ...
      <input name="google_client_id" placeholder="留空则不修改" />

      <label>Client Secret</label>
      <input name="google_client_secret" type="password" placeholder="留空则不修改" />

      <label>Refresh Token</label>
      <input name="google_refresh_token" type="password" placeholder="留空则不修改" />

      <label>Redirect URI</label>
      <input name="google_redirect_uri" placeholder="留空则清除" value="${escapeHtml(cfg.google_redirect_uri || "")}" />
      ...
      <div class="security-notice">
        ...
        <ul>
          ...
          <li>如需更高安全性，建议<a href="https://github.com/openilink/openilink-app-google">自行部署</a>本 App</li>
        </ul>
      </div>
